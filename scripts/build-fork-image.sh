...
# default tag: zankich/vllm-openai:0.29.0z
#
# regenerates forks/v0.29.0z.patch from the current tree (so a freshly
...
TAG="${1:-zankich/vllm-openai:0.29.0z}"

# the diff is from vanilla vLLM (the upstream base) to the fork tip.
# the upstream base is the vLLM v0.29.0 image, NOT this branch.
...
# against the pushed fork tip so uncommitted working-tree churn
# never enters the patch.
mkdir -p forks
git diff v0.29.0 origin/v0.29.0z -- vllm/ >forks/v0.29.0z.patch
echo "patch: forks/v0.29.0z.patch ($(wc -l <forks/v0.29.0z.patch) lines)"
