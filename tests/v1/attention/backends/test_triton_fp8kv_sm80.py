...
def test_quantize_kv_e5m2_rounds_ties_to_even():
    # In e5m2, values between representable points round RNE; 0.51953125
    # (between 0.5 and 0.546875) ties toward the even mantissa -> 0.5.
    key = torch.tensor([0.51953125], dtype=torch.float32)
...
def test_triton_backend_selects_e5m2_below_sm89():
    ...
    assert (
        kv_fp8_dtype_for_platform(
            torch.float8_e4m3fn, platform=_FakePlatform(86)
        )
...
def test_triton_backend_keeps_default_dtype_on_sm89_plus():
    ...
    for cap in (89, 90, 100):
        assert (
            kv_fp8_dtype_for_platform(
                torch.float8_e4m3fn, platform=_FakePlatform(cap)
            )
