...
# Re-pin:   regenerates forks/v0.29.0z.patch automatically before build

# Step 1: start from upstream vLLM v0.29.0
FROM vllm/vllm-openai:v0.29.0

# Step 2: apply the fork patches. The diff carries vllm/ source
# changes only.
COPY forks/v0.29.0z.patch /tmp/v0.29.0z.patch
RUN cd /usr/local/lib/python3.12/dist-packages \
    && patch -p1 --fuzz=0 < /tmp/v0.29.0z.patch \
    && rm /tmp/v0.29.0z.patch
...
print("optin installed:", callable(getattr(fi, "install_sm8_fp8_large_head_optin", None)))
