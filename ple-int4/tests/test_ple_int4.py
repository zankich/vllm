...
def _reference_rows(table: torch.Tensor, ids: torch.Tensor, group_size: int) -> torch.Tensor:
...
def test_kernel_bit_exact_vs_reference():
    ...
    lookup_ple_int4_from_pinned(
        uva_w, uva_s, ids, out,
        vocab_start=0, vocab_end=rows, group_size=GROUP_SIZE,
    )
    ref = _reference_rows(table, ids.cpu(), GROUP_SIZE).cuda()
    assert torch.equal(out, ref), f"max abs diff {(out.float() - ref.float()).abs().max()}"

    # Out-of-range ids -> exact +0.0 (not -0.0)
    ids2 = torch.tensor([rows + 5, rows + 6, 123456789], dtype=torch.int64, device="cuda")
    out2 = torch.empty(3, DIM, dtype=torch.bfloat16, device="cuda")
    lookup_ple_int4_from_pinned(
        uva_w, uva_s, ids2, out2,
        vocab_start=0, vocab_end=rows, group_size=GROUP_SIZE,
    )
...
def test_kernel_shard_partitioning_exact_zeros():
    ...
    for start, end in ((0, half), (half, rows)):
        ...
        lookup_ple_int4_from_pinned(
            uva_w, uva_s, ids, out,
            vocab_start=start, vocab_end=end, group_size=GROUP_SIZE,
        )
        outs.append(out)
...
def test_dispatch_marker_and_env():
    ...
    try:
        ...
        m8 = ng.Qwen4ExpPLEEmbeddingMethod.from_quant_config(None, "prefix", "float8_e4m3fn")
