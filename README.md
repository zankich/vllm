...
| `Bind fs-tier blocks to their keys, detect replaced storage` | the tier's files were content-addressed by path only: full-length wrong-for-key bytes restored silently (a real incident class — index/path confusion, replaced storage, pruner-damaged trees). Each store writes a key-bound checksum sidecar; loads verify before and after the read, and a payload whose storage identity changed under a live engine rejects the whole job to a cold recompute. Never partial trust | fork-local |
