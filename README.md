...
| [`[Bugfix][KV Offload] Stop offloading the final sampled token's KV slot` (#54288)](https://github.com/vllm-project/vllm/pull/54288) | the finished-request offload path stored the final sampled token's KV slot, never written by any forward pass — under spec decode it can hold a rejected draft's KV, content-hash-keyed so replays loaded it as legitimate. A no-op for `offload_prompt_only` configs (the prompt clamp already capped stores) but carried for upstream alignment | upstream, cherry-picked from main |
